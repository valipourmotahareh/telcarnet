...
class CurLocationHelper implements LocationListener {
  ...
  private final GLMapView mapView;
  Location location;

  CurLocationHelper(GLMapView mapView,double offlinelat,double offlinelot) {
    this.mapView = mapView;
    location=new Location("");
    location.setLatitude(offlinelat);
    location.setLongitude(offlinelot);
    if (location == null) return;

    lastLocation = location;
    final MapPoint position =
            MapPoint.CreateFromGeoCoordinates(location.getLatitude(),location.getLongitude());
      this.mapView.animate(animation -> animation.flyToPoint(position));
       this.mapView.setMapZoom(15);
    ...
    if (userLocationImage == null) {
      ...
      if (locationImage != null) {
        ...
        this.mapView.add(userLocationImage);
    ...
    if (userMovementImage == null) {
      ...
      if (movementImage != null) {
        ...
        this.mapView.add(userMovementImage);
    ...
    if (accuracyCircle == null) {
      ...
      accuracyCircle.setVectorObject(circle,
              ...
              null);
      this.mapView.add(accuracyCircle);
    }
